...
class Encoder(nn.Module):
    ...
    def forward(self, x, edge_index, batch, percent):
        ...
        if percent:
            ...
            sampling_subgraph_, assignment_prob = self.multi_head_subgraph_generation(pos_global_node, batch)
        ...
        return pos_global_graph, sampling_subgraph_.reshape(-1, sampling_subgraph_.size(2)), neg_global_graph, prob_loss
    ...
    def multi_head_subgraph_generation(self, node_embeddings, batch):
        ...
        subgraph_node_embed = (node_embeddings * mask)
        # subgraph_embed = self.subgraph_conv(subgraph_node_embed).squeeze()
        sampling_subgraph_list = []
        for i in range(subgraph_node_embed.size(0)):
            sampling_subgraph_list.append(global_add_pool(subgraph_node_embed[i], batch))
        sampling_subgraph_ = torch.stack(sampling_subgraph_list, dim=1).unsqueeze(-1)
        # return subgraph_node_embed, P
        return sampling_subgraph_, P
...
class GraphEnhance(nn.Module):
    ...
    def forward(self, x, edge_index, batch, percent):
        ...
        pos_global_graph, sampling_subgraph_, neg_global_graph, prob_loss = self.encoder(x,
                                                                                         edge_index,
                                                                                         batch,
                                                                                         percent)
        # =========================================
        ...
        #                                        sub_batch)
        # =========================================
        sub_batch = torch.arange(pos_global_graph.size(0)).unsqueeze(1).repeat_interleave(2 ** self.times, 1).view(-1)
        loss_graph_subgraph, E_neg1, E_neg2, E_pos = losses.local_global_loss_(pos_global_graph, sampling_subgraph_,
                                                                               neg_global_graph, sub_batch,
                                                                               measure='JSD')

        return loss_graph_subgraph, prob_loss, E_neg1, E_neg2, E_pos
