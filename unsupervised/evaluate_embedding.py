...
def evaluate_embedding(embeddings, labels, search=True):
    ...
    return np.mean(logreg_accuracies), np.mean(svc_accuracies), np.mean(linearsvc_accuracies)


if __name__ == '__main__':
    from model import Encoder
    from torch_geometric.datasets import TUDataset
    from utils import move_to, get_embeddings
    ...
    batch_size = 128
    total_dataset = TUDataset(root='/tmp/ENZYMES', name=dataset_name).shuffle()
    total_dataloader = DataLoader(total_dataset, batch_size, shuffle=True)
    num_features = 1
    dim = 64
    num_gc_layers = 4
    GNN = Encoder(num_features, dim, num_gc_layers)

    emb, y = get_embeddings(GNN, total_dataloader)
