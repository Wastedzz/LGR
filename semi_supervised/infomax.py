...
def local_global_loss_(g_enc, l_enc, neg_l_enc, batch, measure):
    '''
    Args:
        l: Local feature map.
        g: Global features.
    ...
    pos_mask = torch.zeros((num_nodes, num_graphs)).to(g_enc.device)
    neg_mask = torch.ones((num_nodes, num_graphs)).to(g_enc.device)
    for nodeidx, graphidx in enumerate(batch):
        pos_mask[nodeidx][graphidx] = 1.
        neg_mask[nodeidx][graphidx] = 0.

    res = torch.mm(l_enc, g_enc.t())
    neg_res = torch.mm(neg_l_enc, g_enc.t())
    ...
    E_neg = (E_neg1 * neg_mask + E_neg2 * pos_mask).mean()
